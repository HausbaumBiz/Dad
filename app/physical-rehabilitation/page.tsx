...
export default function PhysicalRehabilitationPage() {
  ...
  useEffect(() => {
    const fetchBusinesses = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(`/api/businesses/by-page?page=physical-rehabilitation&timestamp=${Date.now()}`)
  ...
  return (
    <CategoryLayout title="Physical Rehabilitation" backLink="/" backText="Categories">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-600 mb-4">No physical rehabilitation service businesses found.</p>
