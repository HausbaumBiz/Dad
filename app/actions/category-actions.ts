...
import { saveBusinessCategoriesToDb } from "@/lib/db-schema"
import type { CategorySelection, CategoryData } from "@/lib/definitions"
...
export async function saveBusinessCategories(categories: CategorySelection[]) {
  try {
    ...
    const hasPetCareCategory = categories.some(
      (cat) =>
    ...
    if (hasPetCareCategory) {
      ...
      const petCareFormats = ["petCare", "pet-care", "pet_care", "Pet Care"]
    ...
    if (success) {
      ...
      const categoryIds = [
        ...new Set(
          categories.map((cat) => {
            const normalized = cat.category.toLowerCase().replace(/\s+/g, "")
            return normalized === "petcare" ? "petCare" : normalized
          }),
        ),
      ]
