...
export default function ConstructionDesignPage() {
  ...
  useEffect(() => {
    ...
    const fetchProviders = async () => {
      try {
        const response = await fetch("/api/businesses/by-page?page=construction-design")
        if (!response.ok) {
        ...
        {loading ? (
                      ...
                      <div className="flex items-center mt-2">
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <svg
                              key={i}
                              className={`w-4 h-4 ${i < (provider.average_rating || 0) ? "text-yellow-400" : "text-gray-300"}`}
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                        ...
                        <span className="text-sm text-gray-600 ml-2">
                          {provider.average_rating?.toFixed(1) || "No ratings"} ({provider.review_count || 0} reviews)
                        </span>
                      </div>
          ...
          <div className="text-center py-10">
            <p className="text-gray-700 mb-4">
              No construction and design businesses found. Be the first to register your business in this category!
            </p>
